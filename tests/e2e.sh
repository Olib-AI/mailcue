...
test_inject() {
  ...
  http POST "${API}/emails/inject" -d "{
    ...
    \"html_body\": \"<h1>Hello</h1><p>This is an injected email.</p>\",
    \"text_body\": \"Hello, this is an injected email.\",
  ...
  # Inject plain-text only
  http POST "${API}/emails/inject" -d "{
    \"mailbox\": \"${TEST_MAILBOX}\",
    \"from_address\": \"noreply@example.com\",
    \"to_addresses\": [\"${TEST_MAILBOX}\"],
    \"subject\": \"Plain Text Inject\",
...
test_email_operations() {
  ...
  # Search
  http GET "${API}/emails?mailbox=${TEST_MAILBOX}&search=Inject+Test"
  local search_total
  ...
  [ "$subject" = "E2E Inject Test" ] && pass "Get email detail — subject matches" || fail "Get email detail" "subject: $subject"
  ...
  echo "$html_body" | grep -q "Hello" && pass "HTML body contains expected content" || fail "HTML body" "got: $html_body"
  ...
  # Delete email (use a bulk-injected one)
  http GET "${API}/emails?mailbox=${TEST_MAILBOX}&search=Bulk+Email+3"
  local del_uid
  del_uid=$(echo "$BODY" | jq -r '.emails[0].uid // empty' 2>/dev/null)
  if [ -n "$del_uid" ]; then
    http DELETE "${API}/emails/${del_uid}?mailbox=${TEST_MAILBOX}"
    [ "$CODE" = "204" ] && pass "Delete email — 204" || fail "Delete email" "code: $CODE"
  else
    fail "Delete email" "could not find bulk email 3 to delete"
  fi
}
...
test_mailbox_emails() {
  ...
  local subject
  subject=$(echo "$BODY" | jq -r '.subject' 2>/dev/null)
  [ "$subject" = "E2E Inject Test" ] && pass "GET /mailboxes/{addr}/emails/{uid} — correct" || fail "Nested email detail" "subject: $subject"
...
test_smtp_send() {
  ...
  http POST "${API}/emails/send" -d "{
    ...
    \"body\": \"Sent via the API send endpoint.\",
    \"body_type\": \"plain\"
  }"
  local msg_id
  ...
  # Verify it arrived
  http GET "${API}/emails?mailbox=${TEST_MAILBOX}&search=SMTP+Send+Test"
  local found
...
try:
    msg = MIMEText('Direct SMTP test from e2e script.')
    msg['Subject'] = 'Direct SMTP E2E Test'
    msg['From'] = 'e2e-test@example.com'
    msg['To'] = '${TEST_MAILBOX}'

    s = smtplib.SMTP('${IMAP_HOST}', ${SMTP_PORT}, timeout=10)
    s.sendmail('e2e-test@example.com', ['${TEST_MAILBOX}'], msg.as_string())
    s.quit()
    print('OK')
except Exception as e:
    print(f'ERR:{e}', file=sys.stderr)
...
" 2>&1)
  ...
  sleep 3
  http GET "${API}/emails?mailbox=${TEST_MAILBOX}&search=Direct+SMTP+E2E"
  local found
  found=$(echo "$BODY" | jq '.total' 2>/dev/null)
...
test_gpg_operations() {
  ...
  http POST "${API}/emails/send" -d "{
    \"from_address\": \"admin@${DOMAIN}\",
    \"to_addresses\": [\"${TEST_MAILBOX}\"],
    \"subject\": \"GPG Signed Email\",
    \"body\": \"This email is signed with PGP.\",
    \"body_type\": \"plain\",
    \"sign\": true
  }"
  ...
  http POST "${API}/emails/send" -d "{
    \"from_address\": \"admin@${DOMAIN}\",
    \"to_addresses\": [\"${TEST_MAILBOX}\"],
    \"subject\": \"GPG Encrypted Email\",
    \"body\": \"This email is encrypted with PGP.\",
    \"body_type\": \"plain\",
    \"encrypt\": true
  }"
  [ "$CODE" -ge 200 ] 2>/dev/null && [ "$CODE" -le 202 ] 2>/dev/null \
  ...
  http POST "${API}/emails/send" -d "{
    \"from_address\": \"admin@${DOMAIN}\",
    \"to_addresses\": [\"${TEST_MAILBOX}\"],
    \"subject\": \"GPG Signed+Encrypted Email\",
    \"body\": \"This email is both signed and encrypted.\",
    \"body_type\": \"plain\",
    \"sign\": true,
    \"encrypt\": true
  }"
  ...
  http GET "${API}/emails?mailbox=${TEST_MAILBOX}&search=GPG+Signed+Email"
  local signed_uid
