...
def _simple_spf_parts(value: str) -> tuple[frozenset[str], str] | None:
    """Canonicalize the simple SPF form generated by MailCue.

    Only unqualified/pass ``mx`` and ``a`` mechanisms followed by one terminal
    ``-all`` or ``~all`` are considered reorder-safe. Includes, redirects,
    explicit IP ranges, and other mechanisms fall back to ordered comparison.
    """
    tokens = value.split()
    if len(tokens) < 2 or tokens[0] != "v=spf1":
        return None
    terminal = tokens[-1]
    if terminal not in {"-all", "~all"}:
        return None
    mechanisms: set[str] = set()
    for token in tokens[1:-1]:
        normalized = token[1:] if token.startswith("+") else token
        if normalized == "mx" or normalized == "a" or normalized.startswith(("mx:", "a:")):
            mechanisms.add(normalized.rstrip("."))
            continue
        return None
